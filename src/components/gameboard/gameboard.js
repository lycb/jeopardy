            ...
            <tr key={rowIndex} >
              {board.map((category, categoryIndex) => (
                ...
                {category.map((question, questionIndex) => (
                  question.point ? <button key={questionIndex} disabled={question.resolved} className={`question-button${question.resolved ? "" : "-active"}`} onClick={(event) => handleOnClick(event, question, categoryIndex, questionIndex)}>{question.point}</button> : null
                ))}
                {isOpen && <PopUpCard
                  content={questionObj}
